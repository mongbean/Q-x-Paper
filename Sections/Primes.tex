...
\begin{theorem}[Existence of a Prime Factorisation]
    $\forall n(x)\in\Qx, \exists S\subset\Qx$ where $\forall a(x)\in S$, $a(x)$ is prime, such that $n(x)=\prod_{a(x)\in S}a(x)$.
\end{theorem}
...
Similarly, either $n_1(x)$ is prime or $m_2(x)n_2(x)=n_1(x)$, where $m_2(x)$ is prime. If this terminates at $n_k(x)$ which is prime, we have $n(x)=n_k(x)\prod_{i=1}^{k}m_i(x)$, and all terms on the right hand side are prime. Then, we can have $S=\{n_k(x)\}\cup\{m_i\mid i\in\bZ \land 0<i\leq k\}$.\\
